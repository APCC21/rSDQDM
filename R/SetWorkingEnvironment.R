...
#' @param NtlCode national code in APEC Data Service Syetem
...
SetWorkingEnvironment  <- function(envfile, override=list()) {
  ...
  data <- lapply(data, function(x) if (is.character(x)) gsubfn("\\$\\((.*?)\\)", data, x) else x)
